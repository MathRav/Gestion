...
public interface mvtotalDao extends JpaRepository<mvtotal, Long>{
  ...
  @Query(value = "SELECT u.id,SUM(u.debit) as debit,SUM(u.credit) as credit,SUM(u.debit)-SUM(u.credit) as solde FROM mvtotal u where MONTH(u.dateMvt)>=?1 AND YEAR(u.dateMvt)>=?2 AND MONTH(u.dateMvt)<=?3 AND YEAR(u.dateMvt)<=?4 AND u.idCompte=?5 ORDER BY u.idCompte")
